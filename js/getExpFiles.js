...
onload = () =>{
    ...
    onValue(ref(database,`Files/Expense/${localStorage.getItem('userId')}/`),snapshot =>{
        document.getElementById('mainDiv').innerHTML = '';
        snapshot.forEach(snap => {
            var snapK = snap.key;
            var snapD = snap.val();

            console.log('from get Exp File.js' + snapK +' ; ' + snapD);

            var dialog =   `<h2 data-key="${snapK}">${snapK}</h2><hr style="color:blue;">`;

            document.getElementById('mainDiv').innerHTML += dialog;
        });
    });
}
...
document.addEventListener('click',e=>{
    ...
    if (e.target.matches('h2')) {
        ...
        localStorage.setItem('currentFile',e.target.dataset.key);
        window.location.replace('getExpManage.html')
    }
