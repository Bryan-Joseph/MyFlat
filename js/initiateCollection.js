...
subb.addEventListener('click', ()=>{
    ...
    onValue(ref(database,`AssociationAdmins/${localStorage.getItem('userId')}`),snapshot =>{
        var association = snapshot.val().association;
        var time = new Date()
        set(ref(database,`collections/${association}/${time.getTime()}`),{
            subject:'Maintinence Collection',
            message:`Please pay ${amount} in ${link} before ${deadline}`
        }).then(()=>{
            swal({
                title: "Success",
                text: "Successfully initiated collection",
                icon: "success",
            }).then(()=>{
